...
describe "Candidate" do
  ...
  context 'enrolling of new student' do
    ...
    it "does not enroll student if there is one with same uic" do
      mock_uic_getter = mock(UicGetter)
      UicGetter.should_receive(:new).twice.and_return(mock_uic_getter)
      mock_uic_getter.should_receive(:get_uic).twice.with('7604242624').and_return(1)
      candidate = Candidate.make(:postal_street => 'Short')
      candidate.tutor = Tutor.make
      candidate.specialization = Specialization.make
      candidate.study = Study.make
      candidate.enroll!(TermsCalculator.this_year_start)
      candidate.enroll!(TermsCalculator.this_year_start).should be_false
    end
  end
