module StudyPlansHelper
  ...
  def obligate_select(plan_subject)
    content_tag('select', options_for_select((1..(@student.specialization.last_possible_exam_semester)), plan_subject.finishing_on), { 'id' =>
      "plan_subject_#{plan_subject.id}",'name' =>
      "plan_subject[#{plan_subject.id}][finishing_on]"}) +
    ". " + t(:message_0, :scope => [:helper, :plans]) +
  ...
  def seminar_select(subjects, plan_subject)
    ...
    result << "&mdash; "
    result << content_tag('select', options_for_select(1..(@student.specialization.last_possible_exam_semester),
  ...
  def voluntary_select(subjects, plan_subject)
    ...
    if plan_subject.id && plan_subject.id < 0
      subjects[0] = [t(:message_1, :scope => [:helper, :plans]), -1]
    end
  ...
  def subject_select(subjects, plan_subject)
    select = ''
    subjects = [[t(:message_2, :scope => [:helper, :plans]), -1]].concat(subjects)
    subjects = [[t(:message_3, :scope => [:helper, :plans]), 0]].concat(subjects)
    ...
    select << "&mdash; "
    select << content_tag('select', options_for_select(1..6,
      plan_subject.finishing_on ), { 'id' =>
      "plan_subject_#{plan_subject.id}_finishing_on", 'name' =>
      "plan_subject[#{plan_subject.id}][finishing_on]"})
    return select
  ...
  def empty_subject_select(subjects)
    select = ''
    subjects = [[t(:message_4, :scope => [:helper, :plans]), -1]].concat(subjects)
    subjects = [[t(:message_5, :scope => [:helper, :plans]), 0]].concat(subjects)
    #plan_subject.subject_id = 0 if plan_subject.subject.is_a?(ExternalSubject)
  ...
  def study_plan_menu(student)
    links = []
    links << link_to_unless_current(t(:message_7, :scope => [:helper, :plans]),
                                     {:controller => 'students',
                                      :action => 'end_study'},
                                      :confirm =>  t(:message_8, :scope => [:helper, :plans]) +
                                      ' ' + t(:message_9, :scope => [:helper, :plans]) + '?'){}

    links << link_to_unless_current(t(:message_10, :scope => [:helper, :plans]),
                                     {:controller => 'students',
                                      :action => 'change_tutor'},
  ...
  def create_study_plan_link(student)
    link_to(t(:message_14, :scope => [:helper, :plans]), {:action => 'create', :id => student},
      :confirm => t(:message_15, :scope => [:helper, :plans]))
  end

#  def final_exam_link(index)
#    link_to(t(:message_16, :scope => [:helper, :plans]), :action => 'final_application')
#  end

  def render_requisite
    render(:partial => "shared/subjects",
           :locals => {:subjects => session[:requisite_subjects],
           :title => t(:message_17, :scope => [:helper, :plans])})
  end
  ...
  def return_to_link
    link_to_remote("<<< " + t(:message_20, :scope => [:helper, :plans]),
